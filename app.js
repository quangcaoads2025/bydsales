...
const state = {
  ...
  editingLead: null, editingUser: null, autoTimeTimer: null
};
...
const BRAND = {
  bydLogo: CFG.BYD_LOGO || "assets/byd-logo.png",
  ngocanhLogo: CFG.NGOCANH_LOGO || "assets/ngocanh-logo.png",
  loginTitle: CFG.LOGIN_BRAND_TEXT || "BYD THÀNH CÔNG CÀ MAU | BYD 4S",
  slogan: CFG.SLOGAN || "TỰ TIN TRÊN MỌI HÀNH TRÌNH",
  copyright: CFG.COPYRIGHT || "© 2026 Hệ Thống Ô Tô Xe Máy Ngọc Anh Cà Mau\nBản quyền thuộc về HuuLeMKT"
};
const copyrightHtml = () => esc(BRAND.copyright).replace(/\n/g, "<br>");
function brandImg(src, alt, cls, fallbackText){
  return `<img class="${cls}" src="${esc(src)}" alt="${esc(alt)}" onerror="this.style.display='none';this.nextElementSibling.style.display='grid'"><span class="${cls} logo-fallback" style="display:none">${esc(fallbackText)}</span>`;
}
...
function timeOnly(v){
  ...
  if(!m) return "";
  const t = `${pad2(m[1])}:${m[2]}:${m[3] || "00"}`;
  // Không hiện 00:00:00 vì đây thường là giờ mặc định của Google Sheet khi chỉ có ngày.
  return t === "00:00:00" ? "" : t;
}
function leadTime(r){
  return timeOnly(
    r?.createdTime || r?.created_time || r?.gioPhatSinh || r?.gio || r?.time || r?.recordedTime ||
    r?.createdDateTime || r?.ngayGioPhatSinh || r?.ngayGio || r?.createdAt || r?.created_at ||
    r?.updatedAt || r?.updated_at || r?.lastUpdate || r?.capNhatCuoi || r?.createdDate
  );
}
function leadDateCell(r){
  const d = fmt(r?.createdDate || r?.createdDateTime || r?.ngayGioPhatSinh || r?.createdAt || r?.created_at);
  const t = leadTime(r);
...
function render(){
  if(state.autoTimeTimer){ clearInterval(state.autoTimeTimer); state.autoTimeTimer=null; }
  ...
  app().innerHTML = `<div class="login-wrap">
    <section class="login-hero">
      <div class="hero-brand-box">
        <div class="hero-logo-box">${brandImg(BRAND.ngocanhLogo,"Ngọc Anh","brand-logo hero-logo","Ngọc Anh")}</div>
        <div class="hero-slogan">${esc(BRAND.slogan)}</div>
      </div>
    ...
    <section class="login-panel"><form class="login-card" id="loginForm">
      <div class="login-brand-head">
        <div class="login-logo-box">${brandImg(BRAND.bydLogo,"BYD Thành Công","brand-logo login-logo","BYD")}</div>
        <div class="login-brand-title">${esc(BRAND.loginTitle)}</div>
      </div>
      ...
      <div class="copyright">${copyrightHtml()}</div>
    </form></section>
  </div>`;
...
function chartCard(title,data){ const max=Math.max(1,...data.map(x=>x[1])); return `<div class="card chart-card"><div class="panel-title"><h3>${title}</h3></div>${data.length?data.map(([k,v])=>`<div class="bar-row"><div class="bar-label" title="${esc(k)}">${esc(k)}</div><div class="bar"><span style="width:${Math.max(4,v/max*100)}%"></span></div><b class="bar-value">${v}</b></div>`).join(""):`<div class="empty">Chưa có dữ liệu</div>`}</div>`; }
function leadsPage(){
...
function dateTimeField(label,name,dateValue,timeValue,isNew=false){
  ...
  return `<div class="field date-time-field"><label>${label}</label><input name="${name}" type="date" value="${esc(d)}" required />
    <input type="hidden" name="createdTime" value="${esc(t)}" data-autotime="${isNew ? "1" : "0"}" />
    <input type="hidden" name="createdDateTime" value="${esc(`${d} ${t}`.trim())}" />
    <div class="auto-time"><span>Giờ ghi nhận tự động</span><b id="createdTimePreview">${esc(t || "--:--:--")}</b></div>
  </div>`;
}
function syncLeadAutoTime(form){
  if(!form) return;
  const dateInput = form.querySelector('[name="createdDate"]');
  const timeInput = form.querySelector('[name="createdTime"]');
  const dateTimeInput = form.querySelector('[name="createdDateTime"]');
  const preview = form.querySelector('#createdTimePreview');
  const isNew = !form.querySelector('[name="id"]')?.value;
  // Chỉ tự chạy giờ cho khách mới. Khi sửa khách cũ, giữ nguyên giờ đã ghi nhận.
  if(timeInput && isNew) timeInput.value = nowTime();
  if(dateTimeInput) dateTimeInput.value = `${dateInput?.value || today()} ${timeInput?.value || ""}`.trim();
  if(preview) preview.textContent = timeInput?.value || "--:--:--";
}
function bindLeadAutoTime(){
  const form = $("#leadForm");
  if(!form) return;
  const timeInput = form.querySelector('[name="createdTime"]');
  const dateInput = form.querySelector('[name="createdDate"]');
  const isAuto = timeInput?.dataset.autotime === "1";
  syncLeadAutoTime(form);
  dateInput?.addEventListener("input",()=>syncLeadAutoTime(form));
  if(isAuto){ state.autoTimeTimer = setInterval(()=>syncLeadAutoTime(form),1000); }
}
...
function bindEvents(){
  ...
  $("#leadForm")?.addEventListener("submit", async e=>{
    ...
    o.gio = o.createdTime;
    o.gioPhatSinh = o.createdTime;
    o.recordedTime = o.createdTime;
    o.ngayGioPhatSinh = o.createdDateTime;
    o.created_at = o.createdDateTime;
    const saved = await api("saveLead",{lead:o});
    toast("Đã lưu khách hàng");
    // Cập nhật tạm trên giao diện ngay lập tức. Dữ liệu chuẩn vẫn lấy lại từ Apps Script bằng loadAll().
    if(saved?.id && !o.id) state.leads.unshift({...o, id:saved.id});
